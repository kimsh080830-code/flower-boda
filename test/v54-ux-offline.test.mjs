...
test('main tab swipe follows bottom-nav order, is finite, and excludes horizontal gesture zones and Android edges', () => {
  assert.match(main, /const MAIN_NAV_TABS = \['events', 'home', 'map', 'encyclopedia'\]/);
  assert.match(shell, /\['events', 'nav-calendar', '행사'\][\s\S]*\['home', 'nav-home', '홈'\][\s\S]*\['map', 'nav-map', '지도'\][\s\S]*\['encyclopedia', 'nav-book', '도감'\]/);
