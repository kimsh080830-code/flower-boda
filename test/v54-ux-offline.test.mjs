...
test('main tab swipe follows bottom-nav order, is finite, and excludes horizontal gesture zones and Android edges', () => {
  assert.match(main, /const MAIN_NAV_TABS = \['home', 'events', 'map', 'encyclopedia', 'my'\]/);
  assert.match(shell, /\['home', 'nav-home', '홈'\][\s\S]*\['events', 'nav-calendar', '행사'\][\s\S]*\['map', 'nav-map', '지도'\][\s\S]*\['encyclopedia', 'nav-book', '도감'\][\s\S]*\['my', 'nav-my', 'MY'\]/);
